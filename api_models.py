...
def get_info(authorization_header=Header(
    default="Basic ",
    description="""authorization header need to contain
    credentials of user (username:password)""")) -> dict:
    """
    ...
    if authorization_string is None:
        ...
        authorization_bytes = authorization_string.group(
        ).split('Basic')[1].strip().encode()
    ...
    if (authorization_decoded is None
